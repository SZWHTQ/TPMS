...
#     model.add(tf.keras.layers.Dense(128, activation=act, input_dim=points_num))
...
if os.path.exists("data.npz"):
    ...
    print("Load data...")
    old_time, force, params = load_force_data()
    np.savez_compressed("data.npz", time=old_time, force=force, params=params)
    time, force = preprocess(old_time, force)
...
plt.plot(time, new_force, label="Target Curve", color="blue")
